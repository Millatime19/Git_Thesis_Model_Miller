...
%  Assign Ctot  (total mass concentration)to the base workspace based on parameter inputs for each node. 
...
if Building_Model == 1 
    if n==0 % Start with sub slab condition 
       q_air = q_air_Slab; 
    end 
    if elapsed/86400 >= BC(1,2) && elapsed/86400 <= BC(2,2) % Continue sub slab condition 
       q_air = q_air_Slab; 
    end
    if elapsed/86400 > BC(2,2) && elapsed/86400 < BC(2,3)   % Turn on VIMS, start VIMS q rate
     q_air = q_air_VIMS;
    end
end   

if Building_Model == 0                                      % Use open air condition if outside model is used (Building model = 0)
     q_air = q_air_Open; 
end
...
% % Export model output data and create text file with data output
...
%% 
figure(34)
plot((1/86400)*massremoved(1:n,1),massremoved(1:n,2)*MW(2),'-o')
hold on
plot((1/86400)*massremoved(1:n,1),massremoved(1:n,3)*MW(1),'-o')
    legend('Mass Removed')
...
plot((1/86400)*massremoved(1:n,1), massremoved(1:n,2)*MW(2), '-o', 'Color', [0, 0, 0.6], 'LineWidth', 1.5, 'DisplayName', 'PCE Model');
hold on
...
fprintf('Final PCE mass removed (Model): %.2f g\n', massremoved(n,3)*MW_PCE*1e6/1e6);
fprintf('Final TCE mass removed (Model): %.2f g\n', massremoved(n,2)*MW_TCE*1e6/1e6);

% Convert model data from moles/sec to ug/sec
% moles/sec * g/mol * 1e6 ug/g = ug/sec
PCE_model_rate = diff(massremoved(1:n,3))./diff(massremoved(1:n,1)) * MW_PCE * 1e6;
TCE_model_rate = diff(massremoved(1:n,2))./diff(massremoved(1:n,1)) * MW_TCE * 1e6;
