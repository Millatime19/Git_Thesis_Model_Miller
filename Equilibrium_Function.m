function [Retardation,totmass,G,W,MC,D_nodes,void,PHASE,calc_H,theory_H] = Equilibrium_Function(G,W,MC,por,por_v_min,MW,VP,ACT,KD,SOL,...
...
% DENSLIQ     =   0.15;     % density of NAPL in grams/cc
% SDENS       =   1.5;     % density of dry soil in gm/cc
% TEMP        =   15;      % temperature of soil (deg C)
% airdiff     =   0.091;   % free-air diffusion coeff. (cm^2/s)

TEMP=TEMP+273;   % Convert to Kelvin
% ACT(5) = 10;  % Force activity for VC
...
void(void<0)=por_v_min;
% fprintf('\nInitial ACT for VC (compound 5): %g\n', ACT(5));
...
%  if the compound is miscible, don't count it - doesn't force a NAPL
    if SOL(N)>0
        PHASE(C4>1)=4;
        G(:,N)=C4*(VP(N)/(82.1*TEMP));
        % if N == 5
        % fprintf('\n5. After G calculation in SOL block:\n');
        % fprintf('G value calculated: %g\n', G(1,N));
        % fprintf('Expected G (C4*VP/RT): %g\n', C4(1)*VP(N)/(82.1*TEMP));
        % 
        % end
...
%Removed, does not make a difference. 
%G(:,NCOMP+1)=VPWAT/(82.1*TEMP);      % H2O gas concentration

% fprintf('\n6. G value for VC before W calculation:\n');
% fprintf('G(1,5): %g\n', G(1,5));
...
% Add final VC check here
% fprintf('\n7. Final values for VC:\n');
% fprintf('G(1,5): %g\n', G(1,5));
% fprintf('W(1,5): %g\n', W(1,5));
% fprintf('Final mass check (mol/cm³):\n');
% fprintf('Gas phase: %g\n', G(1,5)*void(1));
% fprintf('Water phase: %g\n', W(1,5)*MC(1,nc));
% fprintf('Sorbed phase: %g\n', KD(5)*SDENS*G(1,5)*void(1));
% fprintf('Total: %g\n', G(1,5)*void(1) + W(1,5)*MC(1,nc) + KD(5)*SDENS*G(1,5)*void(1));
